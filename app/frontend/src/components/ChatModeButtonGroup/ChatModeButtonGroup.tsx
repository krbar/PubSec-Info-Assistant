        ...
        <div className={`${styles.container} ${className ?? ""}`}>
            ...
            featureFlags?.ENABLE_WEB_CHAT && featureFlags?.ENABLE_UNGROUNDED_CHAT ?
                <ButtonGroup className={`${styles.buttonGroup}`} onClick={onClick} bsPrefix="ia">
                    <Button className={`${defaultValue == ChatMode.WorkOnly? styles.buttonleftactive : styles.buttonleft ?? ""}`} size="sm" value={0} bsPrefix='ia'>{t('work_only')}</Button>
                    <Button className={`${defaultValue == ChatMode.WorkPlusWeb? styles.buttonmiddleactive : styles.buttonmiddle ?? ""}`} size="sm" value={1} bsPrefix='ia'>{t('work_and_web')}</Button>
                    <Button className={`${defaultValue == ChatMode.Ungrounded? styles.buttonrightactive : styles.buttonright ?? ""}`} size="sm" value={2} bsPrefix='ia'>{t('generative_ungrounded')}</Button>
                </ButtonGroup>
            : // If only WorkPlusWeb is enabled, show only WorkPlusWeb and WorkOnly buttons
            featureFlags?.ENABLE_WEB_CHAT && !featureFlags?.ENABLE_UNGROUNDED_CHAT ?
                <ButtonGroup className={`${styles.buttonGroup}`} onClick={onClick} bsPrefix="ia">
                    <Button className={`${defaultValue == ChatMode.WorkOnly? styles.buttonleftactive : styles.buttonleft ?? ""}`} size="sm" value={0} bsPrefix='ia'>{t('work_only')}</Button>
                    <Button className={`${defaultValue == ChatMode.WorkPlusWeb? styles.buttonrightactive : styles.buttonright ?? ""}`} size="sm" value={1} bsPrefix='ia'>{t('work_and_web')}</Button>
                </ButtonGroup>
            ...
            : // If neither WorkPlusWeb nor Ungrounded are enabled, show only WorkOnly button
                <ButtonGroup className={`${styles.buttonGroup}`} onClick={onClick} bsPrefix="ia">
                    <Button className={`${defaultValue == ChatMode.WorkOnly? styles.buttonmiddleactive : styles.buttonmiddle ?? ""}`} size="sm" value={0} bsPrefix='ia'>{t('work_only')}</Button>
                </ButtonGroup>
                }
        </div>
    );
